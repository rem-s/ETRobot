...
#define CYC2_PRIORITY 	TMIN_APP_TPRI + 2 /* 周期タスク２の優先度 */
#define CYC3_PRIORITY 	TMIN_APP_TPRI + 2 /* 周期タスク３の優先度 */
#define CYC4_PRIORITY 	TMIN_APP_TPRI + 2
#define CYC5_PRIORITY   TMIN_APP_TPRI + 2
...
typedef struct tail_type{
	int diff;//差
	int now_angle;//今の角度
	int target_angle;//目標の角度

	float speed;//tailの動くスピード
}tail_type;
