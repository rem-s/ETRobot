...
//車体の角度（theta）が１８０度になったらForwardを１００にする。
//  if(gyro_sensor_value > 150 && Forward > TARGET_FORWARD-1){
    if(theta <= -4.21 + 0.05){
      ev3_speaker_play_tone(NOTE_D6, 1000);
      Forward=100;
      TARGET_FORWARD=100;
      Gyro_offset=6;
      TARGET_GYRO_OFFSET=6;
      }

    if(gyro_sensor_value > 150){
      ev3_speaker_play_tone(NOTE_G6, 1000);
      seesawFlag = 1;
    }
