...
static float TARGET_GYRO_OFFSET = 3.0;
...
//目標のターゲット値
static float TARGET_FORWARD = 90;

//スタートから一定時間たつまでturnを0にする
volatile int count = 0;

//周期タスク6が最初に呼び出されたときのみの処理のためのフラグ
volatile int lookup_init_count = 0;

//ゴール以降用のカラーセンサ値の目標値
volatile int DESIRED_VALUE2 = 8;
...
//ゲートをくぐっている間にバー検知したときにフラグを立てる
volatile int lookup_bar_flag = 0;

//ゲートをくぐっている間にバー検知してからの待ち時間
volatile int lookup_change_section_count = -30;

//ゲートをくぐっている間のバー検知までの待ち時間
volatile int lookup_bar_count = 0;

//タスク1を終了と同時にタスク6を起動したらフラグを立てる
volatile int lookup_flag = 0;

//ルックアップゲート内における前行進を決定するための係数
float front = 1.0;

//バーをくぐった回数をカウントする
volatile int complete_count = 0;

//バーを目的回数分クリアしたときのフラグ
volatile int after_bar_flag = 0;

//ルックアップゲートからガレージまでカウント
volatile int after_bar_count = 0;

//ルックアップの態勢に入る直前の処理のためのフラグ
volatile int before_bar_flag = 0;

//ルックアップゲート以降のDESIRED_VALUEを決めるときのフラグ
int color_value_flag = 0;
...
void initialize()
{
	...
	if(ev3_memfile_load("tariy.wav", &memfile) != E_OK) ev3_led_set_color( LED_ORANGE );//SDカード内の"test.wav"を紐づけ
}
...
//周期タスク7(難所からガレージまでのライントレース)
void cyc_task7(intptr_t exinf)
{
...
//周期タスク6(バー検知後のライントレース)
void cyc_task6(intptr_t exinf)
{
	// memfile_t memfile;//ファイル構造体を作成
	...
	// ev3_speaker_play_file(&memfile, 2000);//音声を再生
	
	
	// ev3_speaker_play_file(, 2100);
	
	if(lookup_init_count++ < 250)
	{
	...
	{
		lookup_init_count = 300;  //上のif文が再び実行されないようにするために300に固定する
		...
		else
		{
			...
			//カラーセンサの値取得
			float color_sensor_filtered_value2;
			float color_sensor_normalize_value2;
			color_sensor_value = ev3_color_sensor_get_reflect(color_sensor);
			filtering(color_sensor_value, &color_sensor_filtered_value2);
			normalization(color_sensor_filtered_value2, &color_sensor_normalize_value2);
			lookup_line_trace(color_sensor_normalize_value2, &turn);
			...
			if(turn*beta > 0)
			{
				right_motor_turn = front * 1;
				left_motor_turn = front * (1 + (turn*beta));
			}
			else if(turn*beta < 0)
			{
				left_motor_turn = front * 1;
				right_motor_turn = front * (1 + (-1*turn*beta));
			}
			else
			{
				left_motor_turn = front * 1;
			...
			//まっすぐにしか進ませないなら以下の2行のコメントアウトを外す
			// left_motor_turn = front * 1;
			// right_motor_turn = front * 1;
			
			//ファイルへの書き込み
			fprintf(file2, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n",
			...
			else ev3_motor_set_power(right_motor, (int)right_motor_turn);
...
void cyc_task5(intptr_t exinf)
{
	...
	//コースR用
	// float error = 0.0;
	// if(theta <= -3.14 + error && first_section)
	// {
		// ev3_speaker_play_tone(NOTE_A4, 50);
		// first_section = 0;
		// second_section = 1;
	// }
	// else if(theta >= 0 + error && second_section)
	// {
		// ev3_speaker_play_tone(NOTE_B4, 50);
		// second_section = 0;
		// third_section = 1;
	// }
	// else if(theta >= 1.57 - 10 + error && third_section)
	// {
		// ev3_speaker_play_tone(NOTE_C4, 50);
		// third_section = 0;
		// forth_section = 1;
	// }
	// else if(theta <= -1.57 + error && forth_section)
	// {
		// ev3_speaker_play_tone(NOTE_D4, 50);
		// forth_section = 0;
		// final_section = 1;
...
void cyc_task4(intptr_t exinf)
{
	...
	if(!bar_flag)
	{
		...
		//測距センサで取得した距離が18cm以下の場合、
		if(sonor < 18) approach_count++;
	...
	else if(!before_bar_flag)
	{
		if(bar_wait++ > 3)
		{
			//バー検知後テールを下ろして三点倒立できたら周期タスク1を終了する
			// if(!color_value_flag)
			// {
				// color_value_flag = 1;
				// DESIRED_VALUE2 = sonor;
			// }
			ev3_stp_cyc(CYC_HANDLER1);
			ev3_sta_cyc(CYC_HANDLER6);
			lookup_flag = 1;
			before_bar_flag = 1;  //再びこのelse if文が実行されないようにするための処理
		}
	}
	
	if(complete_count > 0)  // トリプルの場合 ">4", シングルの場合 ">0"
	{
		lookup_flag = 0;
		after_bar_flag = 1;
		// front = -9.0;  //シングルの場合
	...
	if(complete_count > 0)  // トリプルの場合 ">4", シングルの場合 ">0"
	{
		...
		// left_motor_turn = -8;
		// right_motor_turn = -8;
		// ev3_motor_set_power(left_motor, (int)left_motor_turn);
		// ev3_motor_set_power(right_motor, (int)right_motor_turn);
		
		// //speedが0.1だと立ち上がるのに時間がかかる0.2だと倒れる
		// tail_info.speed = 1.2;
		// tail_info.target_angle = WAKEUP_TAIL_POSITION;
		if(after_bar_count++ > 10)
		{
			wup_tsk(MAIN_TASK);
		}
	}
	...
	// {
		...
		// //speedが0.1だと立ち上がるのに時間がかかる.0.2だと倒れる
		// tail_info.speed = 1.2;
		// tail_info.target_angle = WAKEUP_TAIL_POSITION;
		// if(after_bar_count++ > 10)
		// {
	...
	//バー検知後ルックアップの態勢に入ってからの処理
	if(lookup_flag)
	{
		if(sonor < 10) lookup_bar_count++;
		else lookup_bar_count = 0;
		
		if(lookup_bar_count > 5) lookup_bar_flag = 1;
		
		if(lookup_bar_flag)
		{
			if(lookup_change_section_count++ > 40)  //>75
			{
				// if(complete_count < 4) front *= -1.0;  //トリプルの場合 "<4", シングルの場合はコメントアウト
				lookup_bar_flag = 0;
				lookup_change_section_count = 0;
				complete_count++;
			}
		}
	}
...
{	
	...
	{	
		...
		if(!forth_section && !bar_flag)
		{
			Forward += KFORWARD_FIRST * (TARGET_FORWARD - Forward);
		}
		else if(forth_section && !bar_flag)
		{
			TARGET_FORWARD = 30;
		...
		if(first_section && Forward < TARGET_FORWARD-20) turn = 0;
...
//周期タスク6(4msec周期)
void cyc_handler6(intptr_t exinf)
{
	act_tsk(CYC_TASK6);
}

//周期タスク7(4msec周期)
void cyc_handler7(intptr_t exinf)
{
	act_tsk(CYC_TASK7);
}
