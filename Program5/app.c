...
//ゲートをくぐっている間にバー検知してからストップするまでの時間をカウント
...
void cyc_task6(intptr_t exinf)
{
	...
	else
	{
		// ev3_speaker_set_volume(100);//ボリュームを100に設定
		// ev3_speaker_play_file(&memfile, SOUND_MANUAL_STOP);//音声を再生
		// if(ev3_color_sensor_get_color(color_sensor) == COLOR_BLUE) ev3_speaker_play_tone(NOTE_CS4, 1000);
		
		lookup_init_count = 300;
		...
		else
		{
			...
			// if(turn*beta > 0)
			// {
				// right_motor_turn = front * 2;
				// left_motor_turn = front * (2 + (turn*beta));
			// }
			// else if(turn*beta < 0)
			// {
				// left_motor_turn = front * 2;
				// right_motor_turn = front * (2 + (-1*turn*beta));
			// }
			// else
			// {
				// left_motor_turn = front * 4;
				// right_motor_turn = front * 4;
			// }
			
			// if(front < 0) ev3_speaker_play_tone(NOTE_C4, 4);
			
			left_motor_turn = front * 1;
			right_motor_turn = front * 1;
			
			//ファイルへの書き込み
			fprintf(file2, "%lf,%lf,%lf,%lf,%lf,%lf\n",
...
void cyc_task4(intptr_t exinf)
{
	...
	if(lookup_flag)
	{
		if(sonor < 7) lookup_bar_count++;
		else lookup_bar_count = 0;
		
		if(lookup_bar_count > 8) lookup_bar_flag = 1;
		//sonorが5より小さいとバーに反応しない(本番用はもう少し低いため反応するかも)
		// if(sonor < 7) lookup_bar_flag = 1;
		
		if(lookup_bar_flag)
		{
			if(lookup_change_section_count++ > 75)
			{
